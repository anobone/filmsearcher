...
if (!empty($search_item))
{
    ...
    {
        if (empty($_GET['sort'])) $sorting = "rating";
        switch ($sort){
            case("rating"):
                $sorting = "rating";
                break;
            case("date"):
                $sorting = "YEAR('release')";
                break;
            case("alphabet"):
                $sorting = "title";
                break;
        ...
        if ($show!="collection") $search_media = $con->query("SELECT * FROM media WHERE title LIKE '%$search_item%' $showing ORDER BY $sorting DESC");
    }
}
?>